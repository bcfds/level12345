...
@Mod.EventBusSubscriber(modid = TBCcore.MOD_ID)
public class SanityEventHandler {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof Player) {
            // 如果你有多个能力，确保 ID 是唯一的！
            event.addCapability(ResourceLocation.fromNamespaceAndPath(TBCcore.MOD_ID, "sanity"), new SanityProvider((Player) event.getObject()));
            TBCcore.LOGGER.debug("Sanity capability attached to player.");
        }
    }
    ...
    public static void onPlayerClone(PlayerEvent.Clone event) {
        ...
        event.getOriginal().getCapability(ModCapabilities.PLAYER_SANITY).ifPresent(oldSan -> {
            event.getEntity().getCapability(ModCapabilities.PLAYER_SANITY).ifPresent(newSan -> {
                ...
                TBCcore.LOGGER.debug("Sanity data cloned for player.");
            });
        });
    }
